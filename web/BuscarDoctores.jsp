<%-- 
    Document   : BuscarDoctores
    Created on : 04-jun-2020, 22:47:52
    Author     : Carla
--%>
<%@page import="entidad.Doctor"%>
<%@page import="entidad.Recepcionista"%>
<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>Búsqueda doctores</title>
    </head>
    <link href="Recursos/css/estiloIndex.css" rel="stylesheet" type="text/css"/>
     <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css">
     <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css">
       <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
       <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"></script>
        <title>Bienvenido(a)</title>
        <script>
       function showHint(str)
       {
           if (str.length == 0) {
               document.getElementById("txtHint").innerHTML = "";
               return;
           } else {
               var xmlhttp = new XMLHttpRequest();
               xmlhttp.onreadystatechange = function () {
                   if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                       document.getElementById("txtHint").innerHTML = xmlhttp.responseText;
                   }
               }
               xmlhttp.open("GET", "srvTest2?q=" + str, true);
               xmlhttp.send();
           }
       }
   </script>
        <%
            Doctor u = (Doctor) request.getAttribute("data");
        %>
        <nav class="navbar navbar-expand navbar-light btn-lg disabled" style="background: white">
           <div class="container-fluid collapse navbar-collapse">
             <div class="navbar-header">
               <img src="img/LogoSS.jpg" style="width: 130px; height: auto"/>
             </div>
             <ul class="nav navbar-nav mr-auto mt-2 mt-lg-0">
               <li class="nav-item active"><a class="nav-link" href="MenuRecepcionista.jsp">Inicio</a></li>
               <li class="nav-item"><a class="nav-link" href="">Perfil</a></li>
               <li class="nav-item"><a class="nav-link" href="#">Citas médicas</a></li>
                <li class="nav-item dropdown">
                     <a class="nav-link dropdown-toggle" data-toggle="dropdown">Pacientes</a>
                    <div class="dropdown-menu">
                        <a class="dropdown-item" href="crearCuentaPaciente.jsp">Registrar paciente</a>
                        <a class="dropdown-item" href="BuscarPacientes.jsp">Búsqueda de paciente</a>
                    </div>
                </li>
                <li class="nav-item dropdown">
                    <button class="nav-link dropdown-toggle btn btn-info" data-toggle="dropdown">Doctores</button>
                    <div class="dropdown-menu">
                        <a class="dropdown-item" href="crearCuentaDoctor.jsp">Registrar doctor</a>
                        <a class="dropdown-item" disabled="">Busqueda de doctor</a>
                    </div>
                </li>
               <li class="nav-item"><a class="nav-link" href="#">Reprogramar citas</a></li>
             </ul>
             <ul class="nav navbar-nav navbar-right">
               <li style="margin-top: 10px">Bienvenido(a)</li>
               <li class="btn btn-outline-danger btn-sm" style="margin-left: 20px; font-size: medium"><a class="nav-link" href="IngresoRecep.jsp">Cerrar sesión</a></li>
             </ul>
           </div>
         </nav>
    </head>

    <body>
        <div class="container">        
            <form class="form-inline" action="Controlador?metodo=buscardoc" method="post">
                <input type="hidden" name="metodo" value="buscardoctor">
                <input type="hidden" name="id" >
                <div align="center">
                    <h3>Doctor Registrados</h3>
                    <br>
                    <div> 
                        <label>INGRESE NOMBRE DEL DOCTOR :</label>
                    </div>
                    <div>
                        <form action="#0">
                            <input class="form-control" type="text" id="txt1" onkeyup="showHint(this.value)">
                            <a href="crearCuentaDoctor.jsp" class="btn btn-success btn-lg">+</a>
                        </form>
                    </div>
                </div>
            </form>  

            
            <hr>          
            
        </div>
                
        <div class="tab-pane show fade" id="buscaprofe" role="tabpanel" aria-labelledby="buscaprofe-tab">
            <div class="content-wrapper">
                <p><span id="txtHint"></span></p>
            </div>
        </div>
    </body>
</html>
